...
var _ = framework.CertManagerDescribe("SelfSigned CertificateRequest", func() {
	...
	Context("Self Signed and private key", func() {
		...
		It("should be able to obtain an Ed25519 Certificate backed by a RSA key", func() {
			// Replace previous key secret with the RSA one
			_, err := f.KubeClientSet.CoreV1().Secrets(f.Namespace.Name).Update(context.TODO(), newPrivateKeySecret(
				certificateRequestSecretName, f.Namespace.Name, rootRSAKey), metav1.UpdateOptions{})
			Expect(err).NotTo(HaveOccurred())
		...
		It("should be able to obtain an Ed25519 Certificate backed by a ECSDA key", func() {
			// Replace previous key secret with ECSDA one
			_, err := f.KubeClientSet.CoreV1().Secrets(f.Namespace.Name).Update(context.TODO(), newPrivateKeySecret(
				certificateRequestSecretName, f.Namespace.Name, rootECKey), metav1.UpdateOptions{})
			Expect(err).NotTo(HaveOccurred())
